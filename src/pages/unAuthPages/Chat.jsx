...
const Chat = () => {
  const [messages, setMessages] = useState([
    {
      message: "Hello I am your Virtual Doc",
      sender: "Doc",
      userProfile: logo,
    },
  ]);
  ...
  async function processMessageToChatGPT(chatMessages) {
    const API_KEY = "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";
    ...
    const systemMessage = {
      role: "system",
      content:
        "You are acting as a virtual doctor. Based on the symptons of the user, diagnose possible diseases he may have",
    };
  ...
  return (
    <div className="px-5 md:px-24 mt-10 relative">
      <div className=" w-full overflow-y-scroll p-10 h-[calc(100vh-200px)] mt-5 rounded-xl shadow ">
        {messages.map(({ id, isMyMessage, message, userProfile }, index) => {
          return (
            <div className="m-1 mb-10" key={index}>
              <MessageBox
        ...
        >
          <fieldset className=" absolute w-[80%] flex bottom-3">
            ...
            <button className="bg-green-900 w-64 pr-5 rounded-r-md">
              send
            </button>
          </fieldset>
        </form>
